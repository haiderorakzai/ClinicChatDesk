...
try{
  ...
  if(metaCalls.some(x=>x.url.includes('appsecret_proof=')))throw new Error('App Secret Proof must be opt-in, not sent by default');
}finally{globalThis.fetch=realFetch;}

console.log('ClinicChatDesk v2.6.11 smoke test passed: onboarding wizard, clinic team, automatic Meta Embedded Signup, secure app-review server-side WhatsApp connection, encrypted WhatsApp credentials, webhook subscription, WhatsApp template management, localization, live demo, Revenue Recovery, Cancellation Auto-Fill, and Voice-Note tracking.');fs.rmSync(dir,{recursive:true,force:true});
