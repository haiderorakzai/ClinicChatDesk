...
const dir=fs.mkdtempSync(path.join(os.tmpdir(),'clinicchatdesk-smoke-'));process.env.DATA_DIR=dir;process.env.SUPER_ADMIN_EMAIL='smoke-owner@example.com';process.env.SUPER_ADMIN_PASSWORD='SmokePassword123!';process.env.DEMO_MODE='true';
...
const hours={sun:['09:00','18:00'],mon:['09:00','18:00'],tue:['09:00','18:00'],wed:['09:00','18:00'],thu:['09:00','18:00'],fri:['09:00','18:00'],sat:['09:00','18:00']};db.updateConfig(c.businessId,{opening_hours:hours,recovery_delay_minutes:15,lost_lead_recovery:true,cancellation_autofill:true,voice_notes_enabled:true});db.createService(c.businessId,{name:'Dental Cleaning',price:200,currency:'SAR',duration_minutes:30});
const bundle=db.getBusinessBundle(c.businessId);if(bundle.business.name!=='Smoke Dental'||bundle.services.length!==1)throw new Error('Base clinic smoke failed');
...
console.log('ClinicChatDesk v2 smoke test passed: Lost Lead Recovery, Cancellation Auto-Fill, and Voice-Note tracking.');fs.rmSync(dir,{recursive:true,force:true});
