...
console.info('ClinicChatDesk frontend v2.6.9 loaded');
...
const aiLanguageOptions=['English','Arabic','Urdu','Hindi','French','Spanish','Portuguese','Turkish','Bengali','Punjabi','German','Italian','Malay','Indonesian','Russian','Chinese'];
const durationOptions=[10,15,20,30,45,60,75,90,120,150,180];
...
function niceStatus(s=''){return String(s).replaceAll('_',' ').split(' ').map(x=>x?x[0].toUpperCase()+x.slice(1):x).join(' ')}
function setSelectOptions(select,values,current,labeler=v=>v,{multiple=false}={}){
  if(!select)return;const list=[...values];
  if(multiple&&Array.isArray(current))for(const v of current){if(v&&!list.includes(v))list.push(v)}
  else if(!multiple&&current&&!list.includes(current))list.push(current);
  select.innerHTML=list.map(v=>`<option value="${esc(v)}">${esc(labeler(v))}</option>`).join('');
  if(multiple){const chosen=new Set((Array.isArray(current)?current:[]).map(String));[...select.options].forEach(o=>o.selected=chosen.has(o.value));}
  else if(current)select.value=String(current);
}
function selectedValues(select){return select?[...select.selectedOptions].map(o=>o.value).filter(Boolean):[]}
function populateAiSelects(config){
  setSelectOptions(qs('#aiTone'),aiToneOptions,config?.tone||'Warm & professional');
  setSelectOptions(qs('#sAiTone'),aiToneOptions,config?.tone||'Warm & professional');
  const langs=(config?.languages||[]).length?(config.languages||[]):['English'];
  setSelectOptions(qs('#aiLanguages'),aiLanguageOptions,langs,v=>v,{multiple:true});
  setSelectOptions(qs('#sAiLanguages'),aiLanguageOptions,langs,v=>v,{multiple:true});
}
function renderTimeOptions(selected){const vals=timeOptions.includes(selected)?timeOptions:[...timeOptions,selected].filter(Boolean);return vals.map(v=>`<option value="${esc(v)}"${v===selected?' selected':''}>${esc(v)}</option>`).join('')}
function renderDurationOptions(selected){const vals=durationOptions.includes(Number(selected))?durationOptions:[...durationOptions,Number(selected)||30].filter(Boolean);return vals.sort((a,b)=>a-b).map(v=>`<option value="${v}"${Number(v)===Number(selected)?' selected':''}>${v} min</option>`).join('')}
function hoursEditorMarkup(prefix,config){
  return days.map(d=>{const row=config?.opening_hours?.[d]||null;const isOpen=Array.isArray(row)&&row[0]&&row[1];const open=row?.[0]||'09:00';const close=row?.[1]||'18:00';const attr=prefix==='setup'?'data-setup-day':'data-day';
    return `<div class="hours-editor-row ${isOpen?'':'is-closed'}"><div class="hours-day-block"><strong>${dayLabels[d]}</strong><span>${isOpen?'Patients can book on this day':'Closed all day'}</span></div><div class="field inline-field"><label>Status</label><select ${attr}="${d}" data-part="status" class="hours-status"><option value="open"${isOpen?' selected':''}>Open</option><option value="closed"${!isOpen?' selected':''}>Closed</option></select></div><div class="field inline-field"><label>Opens</label><select ${attr}="${d}" data-part="open" class="hours-time"${!isOpen?' disabled':''}>${renderTimeOptions(open)}</select></div><div class="field inline-field"><label>Closes</label><select ${attr}="${d}" data-part="close" class="hours-time"${!isOpen?' disabled':''}>${renderTimeOptions(close)}</select></div></div>`}).join('');
}
function bindHoursEditor(root){
  root?.querySelectorAll('.hours-status').forEach(sel=>sel.onchange=()=>{const row=sel.closest('.hours-editor-row');const open=sel.value==='open';row?.classList.toggle('is-closed',!open);row?.querySelectorAll('.hours-time').forEach(x=>x.disabled=!open);const note=row?.querySelector('.hours-day-block span');if(note)note.textContent=open?'Patients can book on this day':'Closed all day';});
}
function collectHours(attrName){const opening_hours={};for(const d of days){const status=qs(`[${attrName}="${d}"][data-part="status"]`)?.value;const open=qs(`[${attrName}="${d}"][data-part="open"]`)?.value||'';const close=qs(`[${attrName}="${d}"][data-part="close"]`)?.value||'';opening_hours[d]=status==='open'&&open&&close?[open,close]:null}return opening_hours}
...
function uiDialog({title,subtitle='',kicker='ClinicChatDesk',fields=[],confirmText='Save',danger=false}={}){
  ...
  body.innerHTML=fields.map(f=>{const common=`name="${esc(f.name)}" ${f.required?'required':''} ${f.placeholder?`placeholder="${esc(f.placeholder)}"`:''}`;if(f.type==='textarea')return `<div class="field"><label>${esc(f.label)}</label><textarea ${common} rows="${f.rows||3}">${esc(f.value||'')}</textarea>${f.help?`<p class="field-help">${esc(f.help)}</p>`:''}</div>`;if(f.type==='select')return `<div class="field"><label>${esc(f.label)}</label><select ${common}>${(f.options||[]).map(o=>{const value=typeof o==='string'?o:o.value,label=typeof o==='string'?o:o.label;return `<option value="${esc(value)}"${String(value)===String(f.value??'')?' selected':''}>${esc(label)}</option>`}).join('')}</select>${f.help?`<p class="field-help">${esc(f.help)}</p>`:''}</div>`;return `<div class="field"><label>${esc(f.label)}</label><input ${common} type="${esc(f.type||'text')}" value="${esc(f.value??'')}" ${f.min!==undefined?`min="${esc(f.min)}"`:''} ${f.step!==undefined?`step="${esc(f.step)}"`:''} ${f.inputmode?`inputmode="${esc(f.inputmode)}"`:''}>${f.help?`<p class="field-help">${esc(f.help)}</p>`:''}</div>`}).join('');
...
function renderSetup(){
  ...
  const sh=qs('#setupHours');if(sh){sh.innerHTML=hoursEditorMarkup('setup',c);bindHoursEditor(sh);}
...
async function saveSetupHours(){const opening_hours=collectHours('data-setup-day');await post('/api/clinic/config',{opening_hours});await load();await setSetupStep(5)}
async function saveSetupAI(){await post('/api/clinic/config',{ai_name:qs('#sAiName').value,greeting:qs('#sAiGreeting').value,tone:qs('#sAiTone').value,languages:selectedValues(qs('#sAiLanguages'))});await load();await setSetupStep(6)}

function renderServices(){const cur=data?.business?.currency||'USD';const services=data.services||[];const box=qs('#services');if(!box)return;box.innerHTML=services.length?services.map(s=>`<div class="service-editor"><div class="service-editor-grid"><div class="field"><label>Service name</label><input data-s="${s.id}" data-k="name" value="${esc(s.name)}" placeholder="Dental Cleaning"></div><div class="field"><label>Short description</label><input data-s="${s.id}" data-k="description" value="${esc(s.description||'')}" placeholder="Optional note patients may ask about"></div><div class="field"><label>Price</label><input data-s="${s.id}" data-k="price" type="number" value="${s.price}" min="0" step="0.01"></div><div class="field"><label>Currency</label><div class="currency-chip large">${esc(cur)}</div></div><div class="field"><label>Duration</label><select data-s="${s.id}" data-k="duration_minutes">${renderDurationOptions(s.duration_minutes)}</select></div><div class="field"><label>Status</label><select data-s="${s.id}" data-k="active"><option value="1"${Number(s.active)!==0?' selected':''}>Active</option><option value="0"${Number(s.active)===0?' selected':''}>Hidden</option></select></div></div><div class="service-editor-actions"><button class="btn ghost small save-service" data-id="${s.id}">Save</button></div></div>`).join(''):'<p style="color:var(--muted)">No services yet.</p>';qsa('.save-service').forEach(b=>b.onclick=async()=>{const id=b.dataset.id,obj={};qsa(`[data-s="${id}"]`).forEach(i=>{if(i.dataset.k==='price'||i.dataset.k==='duration_minutes'||i.dataset.k==='active')obj[i.dataset.k]=Number(i.value);else obj[i.dataset.k]=i.value});await post('/api/clinic/services/'+id,obj);await load();showToast('Service updated.');});}
async function addServiceWithModal(after){const cur=data?.business?.currency||'USD';const v=await uiDialog({title:'Add service',subtitle:'Add the service name patients know, an optional note, the price and expected duration.',fields:[{name:'name',label:'Service name',placeholder:'Dental Cleaning',required:true},{name:'description',label:'Short description / notes',placeholder:'Optional note patients may ask about'},{name:'price',label:`Price (${cur})`,type:'number',value:'0',min:'0',step:'0.01',required:true},{name:'duration_minutes',label:'Duration (minutes)',type:'number',value:'30',min:'5',step:'5',required:true}],confirmText:'Add service'});if(!v)return;try{await post('/api/clinic/services',{name:v.name.trim(),description:v.description.trim(),price:Number(v.price||0),duration_minutes:Number(v.duration_minutes||30)});await load();if(after)after();showToast('Service added.')}catch(e){showToast(e.message,'error')}}
...
async function loadConversations(){const r=await api('/api/clinic/conversations');qs('#convList').innerHTML=r.conversations.map(c=>`<div class="conv${currentConv?.id===c.id?' active':''}" data-id="${c.id}" data-human="${c.human_handoff}" data-channel="${esc(c.channel||'')}"><strong>${c.last_message_type==='voice'?'🎤 ':''}${esc(c.customer_name||c.customer_phone)}</strong><small>${esc(c.last_message||'')}</small></div>`).join('')||'<div style="padding:18px;color:var(--muted)">No conversations.</div>';qsa('.conv').forEach(x=>x.onclick=()=>openConv(x.dataset.id,x.dataset.human==='1',x.querySelector('strong').textContent,x.dataset.channel));}
async function openConv(id,human,name,channel='whatsapp'){currentConv={id,human,channel};qs('#convTitle').textContent=name;qs('#handoffBtn').classList.remove('hidden');qs('#handoffBtn').textContent=human?'Return to AI':'Take over';const form=qs('#manualReplyForm'),hint=qs('#manualReplyHint'),send=qs('#manualSend');form?.classList.remove('hidden');const canReply=channel==='whatsapp';if(send)send.disabled=!canReply;if(hint)hint.textContent=canReply?'Sending a manual reply automatically puts this conversation in human takeover mode.':'Manual sending is available for real WhatsApp conversations only.';const r=await api(`/api/clinic/conversations/${id}/messages`);qs('#messages').innerHTML=r.messages.map(m=>`<div class="msg ${m.role==='assistant'?'assistant':'user'}">${m.message_type==='voice'?'<div class="voice-chip">🎤 Voice note transcript</div>':''}${esc(m.text)}</div>`).join('');qs('#messages').scrollTop=qs('#messages').scrollHeight;qsa('.conv').forEach(x=>x.classList.toggle('active',x.dataset.id===id));}
...
qs('#manualReplyForm').onsubmit=async e=>{e.preventDefault();if(!currentConv||currentConv.channel!=='whatsapp')return;const input=qs('#manualReply'),button=qs('#manualSend'),text=input.value.trim();if(!text)return;button.disabled=true;button.textContent='Sending…';try{const r=await post(`/api/clinic/conversations/${currentConv.id}/messages`,{text});currentConv.human=true;qs('#handoffBtn').textContent='Return to AI';input.value='';await openConv(currentConv.id,true,qs('#convTitle').textContent,'whatsapp');await loadConversations();showToast('WhatsApp message sent.');if(r?.warning)showToast(r.warning,'warn')}catch(e){showToast(e.message,'error')}finally{button.disabled=false;button.textContent='Send message'}};
...
function templateStatusClass(status=''){status=String(status).toUpperCase();return status==='APPROVED'?'':(status==='REJECTED'||status==='DISABLED')?'danger':'warn'}
async function loadWhatsAppTemplates(){
  const panel=qs('#waTemplatePanel'),rows=qs('#waTemplateRows'),err=qs('#waTemplateError');if(!panel||!rows||!data)return;const w=data.whatsapp||{};const connected=String(w.status||'')==='connected'&&!data.business?.is_demo;panel.classList.toggle('hidden',!connected);if(!connected)return;err?.classList.add('hidden');rows.innerHTML='<tr><td colspan="5">Loading templates…</td></tr>';
  try{const r=await api('/api/clinic/whatsapp/templates');const items=r.templates||[];rows.innerHTML=items.length?items.map(t=>`<tr><td><strong>${esc(t.name||'')}</strong></td><td>${esc(niceStatus(t.category||''))}</td><td>${esc(t.language||'')}</td><td><span class="status ${templateStatusClass(t.status)}">${esc(niceStatus(t.status||'unknown'))}</span></td><td class="template-body-cell">${esc(templateBody(t)||'—')}</td></tr>`).join(''):'<tr><td colspan="5">No message templates found for this WhatsApp Business Account.</td></tr>';}catch(e){rows.innerHTML='<tr><td colspan="5">Could not load templates.</td></tr>';if(err){err.textContent=e.message;err.classList.remove('hidden')}}
}
async function createWhatsAppTemplate(){
  const values=await uiDialog({title:'Create WhatsApp template',subtitle:'ClinicChatDesk will submit this template to Meta for review.',kicker:'WhatsApp Business Management',fields:[{name:'name',label:'Template name',placeholder:'appointment_reminder_review',required:true,help:'Lowercase letters, numbers and underscores only.'},{name:'category',label:'Category',type:'select',value:'UTILITY',options:[{value:'UTILITY',label:'Utility'},{value:'MARKETING',label:'Marketing'}],required:true},{name:'language',label:'Language code',value:'en_US',required:true,help:'Examples: en_US, en_GB, ur, ar, hi.'},{name:'body',label:'Message body',type:'textarea',rows:5,value:'Hello, this is a reminder about your upcoming appointment. Please reply if you need any assistance.',required:true,help:'You may use {{1}}, {{2}}, etc. If you do, add one sample per line below.'},{name:'samples',label:'Variable samples (optional)',type:'textarea',rows:3,placeholder:'Ahmed\n25 August 2026 at 3:00 PM',help:'One sample value per line, in the same order as {{1}}, {{2}}, etc.'}],confirmText:'Submit to Meta'});
  if(!values)return;const button=qs('#waCreateTemplate');if(button){button.disabled=true;button.textContent='Submitting…'};try{const samples=String(values.samples||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);const r=await post('/api/clinic/whatsapp/templates',{name:values.name.trim(),category:values.category,language:values.language.trim(),body:values.body.trim(),samples});showToast(`Template submitted to Meta${r.template?.status?` — ${niceStatus(r.template.status)}`:''}.`);await loadWhatsAppTemplates()}catch(e){showToast(e.message,'error')}finally{if(button){button.disabled=false;button.textContent='+ Create template'}}
}
qs('#waRefreshTemplates').onclick=loadWhatsAppTemplates;qs('#waCreateTemplate').onclick=createWhatsAppTemplate;
