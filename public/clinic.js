...
console.info('ClinicChatDesk frontend v2.6.0 loaded');
...
function renderStaff(){
  ...
  qsa('.edit-staff').forEach(btn=>btn.onclick=async()=>{const x=items.find(y=>y.id===btn.dataset.id);if(!x)return;const name=prompt('Name',x.name);if(!name)return;const specialty=prompt('Specialty / role',x.specialty||'')??x.specialty;await post('/api/clinic/staff/'+x.id,{name,specialty});await load();page('team')});
}
...
const addTeam=async()=>{const name=prompt('Doctor / staff name');if(!name)return;const specialty=prompt('Specialty / role (optional)')||'';await post('/api/clinic/staff',{name,specialty});await load();page('team')};qs('#addTeamMember').onclick=addTeam;
qs('#setupAddService').onclick=async()=>{const name=prompt('Service name');if(!name)return;const price=Number(prompt(`Price in ${data?.business?.currency||'USD'}`)||0);const duration_minutes=Number(prompt('Duration in minutes')||30);await post('/api/clinic/services',{name,price,duration_minutes});await load();setupStep=2;renderSetup()};
qs('#setupAddStaff').onclick=async()=>{const name=qs('#setupStaffName').value.trim();if(!name)return alert('Enter a doctor or staff name.');await post('/api/clinic/staff',{name,specialty:qs('#setupStaffSpecialty').value.trim()});qs('#setupStaffName').value='';qs('#setupStaffSpecialty').value='';await load();setupStep=3;renderSetup()};
...
qsa('[data-setup-next]').forEach(b=>b.onclick=async()=>{const step=Number(b.dataset.setupNext);try{if(step===1)return await saveSetupDetails();if(step===2){if(!(data.services||[]).length)return alert('Add at least one service before continuing.');return await setSetupStep(3)}if(step===3)return await setSetupStep(4);if(step===4)return await saveSetupHours();if(step===5)return await saveSetupAI();if(step===6)return await setSetupStep(7)}catch(e){alert(e.message)}});
qs('#setupConnectExisting').onclick=()=>{page('whatsapp');startWhatsAppSignup('existing')};qs('#setupConnectNew').onclick=()=>{page('whatsapp');startWhatsAppSignup('new')};qs('#setupLiveTest').onclick=()=>page('test');qs('#finishSetup').onclick=async()=>{try{data.onboarding=await post('/api/clinic/onboarding',{step:7,complete:true});await load();page('overview')}catch(e){alert(e.message)}};
